...
if __name__ == "__main__":
    ...
    symbol = "BTC/USDT"
    intervalle = "1h"
    eventlog = "Test intégration récupération et log signal brut"

    # Liste des indicateurs à récupérer en bulk (optimisé pour le plan Free)
    indicateurs_bulk = ["rsi", "macd", "bbands", "ema", {"indicator": "rsi", "optInTimePeriod": 5}]

    # Récupération des indicateurs via l'API taapi.io (bulk)
    ...
    # Construction du signal brut à logger
    signal = {
        "symbol": symbol,
    ...
    # DEBUG : Affichage du contenu de raw_json et test de sérialisation
    try:
        print("[DEBUG] raw_json before insert:", signal["raw_json"])
        json.dumps(signal["raw_json"])  # Doit passer sans erreur
    except Exception as e:
        print("[DEBUG] Erreur de sérialisation JSON:", e)
        import sys; sys.exit(1)

    # Insertion en base PostgreSQL
    test_name = "integration_signal_to_db_1h"
    try:
        logger = PgLogger()
