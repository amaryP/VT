...
def fetch_indicators(symbol, interval, api_key=None, debug=False):
    ...
    indicators = [
        ...
        {"indicator": "ma", "optInTimePeriod": 20},  # MA20 TrendSwing US
        {"indicator": "ma", "optInTimePeriod": 50},  # MA50 TrendSwing US
        {"indicator": "ma", "optInTimePeriod": 20, "field": "volume"},  # MA(volume 20) TrendSwing US
    ...
    try:
        ...
        for entry in data.get("data", []):
            ...
            if indicator == "ema":
                ...
                id_parts = id_str.split("_")
                for i, part in enumerate(id_parts):
                    if part == "ma" and i+1 < len(id_parts):
                        try:
                            period = int(id_parts[i+1])
                        except Exception:
                            period = None
                        break
                if period == 20 and res.get("field") == "volume":
                    result["ma_volume_20"] = res.get("value")
                elif period == 20:
                    result["ma20"] = res.get("value")
                elif period == 50:
                    result["ma50"] = res.get("value")
            elif indicator == "adx":
