...
WISHLISTS = {
    'crypto': os.path.abspath(os.path.join(SRC_DIR, '../../wishlist_symbols_crypto.txt')),
    'usstock': os.path.abspath(os.path.join(SRC_DIR, '../../wishlist_symbols_usstock_nasdaq.txt')),
    'nasdaq': os.path.abspath(os.path.join(SRC_DIR, '../../wishlist_symbols_usstock_nasdaq.txt')),
}

def load_symbols(asset_type):
    path = WISHLISTS.get(asset_type)
    if not path or not os.path.exists(path):
        raise ValueError(f"Type d'actif inconnu ou fichier manquant: {asset_type}")
    with open(path, 'r', encoding='utf-8') as f:
        return [line.strip() for line in f if line.strip() and not line.startswith('#')]

def collect_and_analyse(asset_type, interval='15m'):
    # 1. Collecte
    symbols = load_symbols(asset_type)
    now = datetime.now(timezone.utc).replace(second=0, microsecond=0)
    out_name = f"{asset_type}_{interval}_{now.strftime('%Y%m%dT%H%M')}.csv"
    out_path = os.path.join(DATA_DIR, out_name)
    # Import dynamique de fetch_indicators
    ...
    # 2. Analyse
    # Import dynamique analyse_signaux
...
def main():
    parser = argparse.ArgumentParser(description="Collecte et analyse pour crypto ou usstock.")
    parser.add_argument('--type', choices=['crypto', 'usstock', 'nasdaq'], required=True, help='Type d\'actif à traiter')
    args = parser.parse_args()
    collect_and_analyse(args.type)
