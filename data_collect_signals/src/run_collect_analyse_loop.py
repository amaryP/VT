...
def main_loop():
    ...
    analyse_path = os.path.join(SRC_DIR, 'analyse_signaux.py')
    spec = importlib.util.spec_from_file_location("analyse_signaux", analyse_path)
    analyse_signaux = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(analyse_signaux)
    while True:
        now = datetime.datetime.now(PARIS_TZ)
