            ...
            try:
                ...
                for entry in hist.get("raw_json", {}).get("data", []):
                    ...
                    if indicator == "volume":
                        ...
                        elif isinstance(result.get("close"), list):
                            closes_hist.extend([v for v in result["close"] if v is not None])
                        print(f"[DEBUG] {symbol} candle values: {values if values else result.get('close')}")
                    elif indicator == "rsi":
                        values = result.get("values")
                        if values:
                        ...
                        print(f"[DEBUG] {symbol} rsi values: {values if values else result.get('value')}")
            except Exception as e:
            ...
            for i in range(0, len(all_pattern_indicators), 20):
                ...
                for entry in patterns_result.get("raw_json", {}).get("data", []):
                    ...
                    if val in (100, -100, "100", "-100"):
                        pattern_found = True
            signal["pattern"] = "CALL" if pattern_found else (signal.get("pattern") or indicateurs.get("pattern"))
            # Ajout des patterns booléens au signal
